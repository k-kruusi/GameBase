...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        background.zPosition = -1             //depth of background, -1 makes it go behind other 2D elements
        background.position = CGPoint(x: size.width / 2, y: size.height / 2 )          //centering the background images
        background.setScale(1.43)             //Maximizing the pixel resolution for the 1080p
        addChild(background)                  //adding the background to the scene
        
        addChild(player)                      //adding the spaceship to the scene
        player.position = CGPoint(x: 220 , y: size.height/2)              //positioning the spaceship to spawn on the centre of the leftmost corner
        player.setScale(0.6)                  //scale the spaceship down
